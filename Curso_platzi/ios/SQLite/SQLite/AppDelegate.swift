...
// inicia con guin bajo poque se manda llamar algo que no tiene ninguna etiqueta
    ...
    func cargarDB(_ nombreDB: String){
        ...
        let pathDbEnBundle = objetoFileHelper.pathArchivoEnCarpetaDocumentos(nombreDB)
        
        if objetoFileHelper.existeArchivoEnDocumentos(nombreDB)
        {
            print("LA DB YA EXISTE EN LA CARPETA DE DOCUMENTOS")
        }else{
            do{
                try objetoFileManager.copyItem(atPath: pathDbEnBundle, toPath: pathDbEnDocumentos)
            }catch _{
             print("No se puede copiar")
            }
        }
    }
